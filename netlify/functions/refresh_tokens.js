...
import { listConnectionsForRefresh, updateConnection } from '../../shared/storage.js';

/**
 * Refresh long/short lived tokens where supported (Google, TikTok; Meta uses 60-day tokens, refresh via extension).
 */
export async function handler() {
  const conns = await listConnectionsForRefresh();
  const now = Date.now();
    ...
    try {
      if (c.provider === 'youtube' && c.refresh_token && (!c.expires_at || now > c.expires_at - 5 * 60 * 1000)) {
        ...
        if (tokenResp.access_token) {
          await updateConnection(c.id || 'youtube', {
            access_token: tokenResp.access_token,
            expires_at: now + tokenResp.expires_in * 1000
          }, {
            user_id: c.user_id,
